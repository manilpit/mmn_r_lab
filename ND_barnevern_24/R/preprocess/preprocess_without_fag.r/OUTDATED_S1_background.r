...
# Step 1.1: Create the 'unzipped' folder if it doesn't exist (with recursive = TRUE)
...
# Step 3: Create the 'unzipped_csv' folder if it doesn't exist (with recursive = TRUE)
...
# Step 4: Use list.files to list all .txt or .TXT files in the unzipped folder (case-insensitive)
# and exclude files containing "fag" in the filename
...
# Step 5: Check if any .txt or .TXT files were found after filtering
if (length(txt_files) == 0) {
  print("No .txt or .TXT files found in the unzipped folder after filtering out 'fag' files.")
} else {
  # Step 6: Convert each .txt file to a .csv file and save in the unzipped_csv folder
